...
var GetOrganizations = React.createClass({
    ...
    addInput: function(){
        if(addOrg == true){
            ...
            cbs.push(React.createElement(Add_Qualification,{ key: x }));
            this.setState({ chatboxes: cbs });
            x++;
        }

        $('.wrapper').on('click', '.remove_btn_org', function(e){ //Once remove button is clicked
            e.preventDefault();
            addOrg = true;
    ...
    componentDidMount() {
        $.get("/api/v1/organizations/student/"+id, function(result) {
            this.setState({
                data: result
            });
        }.bind(this));
        ...
        return React.createElement(
             ...
             React.createElement(
                    ...
                    React.createElement(
                        "h3",
                        { className: "box-title" },
                        React.createElement("i", { className: "fa fa-users margin-r-5" }),
                        "Organization"
                    ),
                    React.createElement(
                        "a",
...
var Organization_list = React.createClass({
    displayName: "Organization_list",

    render: function render() {
        var QualOpt = function QualOpt(result) {
            return React.createElement(Orgs,
                {key: result.id,
                data: result
            });
        };
...
var Orgs = React.createClass({
    ...
    getInitialState: function(){
        return{
            ...
            user_organization: this.props.data.user_organization,
            olduser_organization: this.props.data.user_organization,
            show: false
        };
    },
    handleEsc: function(event){
        if(event.keyCode == 27){
            this.setState({
                user_organization : this.state.olduser_organization,
                show : false
            });
        }
    },
    ...
    handleSubmit(e) {
        ...
        formData.append('user_organization', this.state.user_organization);
    ...
    deleteToServer(deleteData) {
        $.ajax({
            type: "DELETE",
            url: "/api/v1/organizations/delete/"+this.state.value,
            data: deleteData,
            processData: false,
            contentType: false,
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success == false) {
                    ...
                    this.setState({
                        show :  false,
                        olduser_organization : data.user_organization
                    })
                    // console.log(data);
                }
            }.bind(this)
    ...
    render: function render() {
        if(this.state.show == false){
            ...
            return React.createElement(
                ...
                React.createElement(
                    "input",
                    {type:"text", className:"form-works", placeholder:"Type your organization here!", value: this.state.user_organization, name:"user_organization", onChange: this.handleChange.bind(this, 'user_organization'), onKeyDown: this.handleEsc, autoFocus: true, required: "required"}
                ),
                React.createElement(
                    "a",
...
var Add_Qualification = React.createClass({
    getInitialState: function(){
        return {
            data: [],
            new_organization: '',
            user_organization: '',
            olduser_organization: '',
            show: 1
        };
    },
    showEdit: function(){
    ...
    handleSave(e) {
        ...
        saveData.append('user_organization', this.state.user_organization);
    ...
    handleSubmit(e) {
		e.preventDefault();
        var formData = new FormData();
        formData.append('new_organization', this.state.new_organization);
        formData.append('id', this.state.value);
    ...
    deleteToServer(deleteData) {
        $.ajax({
            type: "DELETE",
            url: "/api/v1/organizations/delete/"+this.state.id,
            data: deleteData,
            processData: false,
            contentType: false,
            success: function(data) {
    ...
    render() {
        if(this.state.show == 0){
            ...
            return React.createElement(
                ...
                React.createElement(
                    "input",
                    {type:"text", className:"form-works", placeholder:"Type your organizations here!", name:"new_organization", value: this.state.new_organization, onChange: this.handleChange.bind(this, 'new_organization'), autoFocus: true , required: "required"}
                ),
                React.createElement(
                    "a",
                    {className:"btn btn-del pull-right btn-size-save remove_btn_org", title: "Cancel"},
                    React.createElement(
                        "span",
            ...
            return React.createElement(
                ...
                React.createElement(
                    "input",
                    {type:"text", className:"form-works", placeholder:"Type your organizations here!", value: this.state.user_organization, name:"user_organization", onChange: this.handleChange.bind(this, 'user_organization'), autoFocus: true, required: "required", onKeyDown: this.handleEsc}
                ),
                React.createElement(
                    "a",
                    {className:"btn btn-del pull-right btn-size-save", onClick: this.handleCancel, title: "Cancel"},
...
var Content = React.createClass({
	render() {
		return React.createElement(
            "div",
			null,
			React.createElement(GetOrganizations)
		)
	}
});
...
ReactDOM.render(React.createElement(
   ...
   document.getElementById('user-organizations')
);
